...
#
# # A = [[63, 51, 0, 8, 51, 50, 45, 0, 24, 0, 31, 8, 0, 0], [55, 51, 0, 8, 3, 50, 13, 0, 24, 0, 31, 8, 0, 0]]
# # B = [[0, 0, 59, 0, 0, 13, 0, 63, 1, 55, 0, 0, 63, 63], [0, 0, 58, 0, 0, 12, 0, 63, 0, 37, 0, 0, 63, 63]]
# #
# # print(A[-1])
# # print(A[len(A)-1])
#
...
import pandas as pd
#
# List = open("SMSSpamCollection.txt").readlines()

List1 = open("spam.txt").readlines()

print(len(List1))

List = open("ham.txt").readlines()
List2 = []
for lis in List:
    List2.append('ham '+lis)
...
for ij in List2:
 splitted = ij.split()
 if splitted[0] == 'ham':
  # print(ij.replace(splitted[0], '').strip())
  model_results['v1'].append('ham')
  model_results['v2'].append(ij.replace(splitted[0], '').strip())
 elif splitted[0] == 'spam':
  model_results['v1'].append('spam')
  model_results['v2'].append(ij.replace(splitted[0], '').strip())
  # print(ij.replace(splitted[0], '').strip())

pd.DataFrame(model_results).to_csv('British English SMS Corpora.csv', index=False)
